...
type NormalizeDurableObjectMethods<DURABLE_OBJECT> = {
  [KEY in keyof DURABLE_OBJECT]: DURABLE_OBJECT[KEY] extends (
    body: infer FIRST_ARG,
    ...args: any[]
  ) => infer RETURN_TYPE
    ? (request: FIRST_ARG) => RETURN_TYPE
    : never;
};
...
export function getDurableObjectInstance<DURABLE_OBJECT extends {}>(
  ...
  name: string,
  context: {},
): NormalizeDurableObjectMethods<DURABLE_OBJECT> {
  return new Proxy(
    {} as unknown as NormalizeDurableObjectMethods<DURABLE_OBJECT>,
    {
      get(_, method: string): any {
        return (argument: any) => {
          ...
          return durableObjectNamespace
            .get(id)
            .fetch(
              `https://${durableObject.serviceName}.durable-object/${method}`,
              {
