...
export const GetPetResponse = {
  encode(
    ...
    if (message.pet !== undefined) {
      Image.encode(message.pet, writer.uint32(10).fork()).ldelim();
    }
    return writer;
  },
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): GetPetResponse {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.pet = Image.decode(reader, reader.uint32());
          break;
        default:
          reader.skipType(tag & 7);
          break;
  ...
  fromJSON(object: any): GetPetResponse {
    return {
      pet: isSet(object.pet) ? Image.fromJSON(object.pet) : undefined,
    };
  },
  ...
  toJSON(message: GetPetResponse): unknown {
    ...
    message.pet !== undefined &&
      (obj.pet = message.pet ? Image.toJSON(message.pet) : undefined);
    return obj;
  },
