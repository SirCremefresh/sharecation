...
class PublicJwk$Type extends MessageType<PublicJwk> {
    ...
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: PublicJwk): PublicJwk {
    ...
    internalBinaryWrite(message: PublicJwk, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* string kid = 3; */
        if (message.kid !== "")
...
class PublicJwks$Type extends MessageType<PublicJwks> {
    constructor() {
        super("authentication.v1.PublicJwks", [
            { no: 1, name: "jwks", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => PublicJwk }
        ]);
    }
    ...
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: PublicJwks): PublicJwks {
        let message = target ?? this.create(), end = reader.pos + length;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
                case /* repeated authentication.v1.PublicJwk jwks */ 1:
                    message.jwks.push(PublicJwk.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                default:
                    let u = options.readUnknownField;
                    if (u === "throw")
    ...
    internalBinaryWrite(message: PublicJwks, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* repeated authentication.v1.PublicJwk jwks = 1; */
        for (let i = 0; i < message.jwks.length; i++)
            PublicJwk.internalBinaryWrite(message.jwks[i], writer.tag(1, WireType.LengthDelimited).fork(), options).join();
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
...
class GetPublicJwksResponse$Type extends MessageType<GetPublicJwksResponse> {
    constructor() {
        super("authentication.v1.GetPublicJwksResponse", [
            { no: 1, name: "ok", kind: "message", oneof: "response", T: () => PublicJwks },
    ...
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: GetPublicJwksResponse): GetPublicJwksResponse {
        let message = target ?? this.create(), end = reader.pos + length;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
                case /* authentication.v1.PublicJwks ok */ 1:
                    message.response = {
                        oneofKind: "ok",
                        ok: PublicJwks.internalBinaryRead(reader, reader.uint32(), options, (message.response as any).ok)
                    };
                    break;
    ...
    internalBinaryWrite(message: GetPublicJwksResponse, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* authentication.v1.PublicJwks ok = 1; */
        if (message.response.oneofKind === "ok")
            PublicJwks.internalBinaryWrite(message.response.ok, writer.tag(1, WireType.LengthDelimited).fork(), options).join();
